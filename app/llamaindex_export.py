...
@trace_func
def _safe_imports():
    """Import optional LlamaIndex APIs. Return (SimpleDirectoryReader, PDFReader) or (None, None).
    Marked with type ignores to keep Pylance quiet when the package isn't installed.
    """
    try:
    ...
    for p in paths:
        try:
            ...
            try:
                ...
                if img_dir.exists():
                    # Mirror to elements/llamaindex/[doc]/images
                    for img in img_dir.glob("*.png"):
                        if img.name.lower().startswith(stem_norm):
                            shutil.copy2(img, stem_dir/"images"/img.name)
                    # Also mirror under data/images/llamaindex for global browsing
                    li_img_root = Path("data")/"images"/"llamaindex"
                    (li_img_root).mkdir(parents=True, exist_ok=True)
                    for img in img_dir.glob("*.png"):
                        if img.name.lower().startswith(stem_norm):
                            shutil.copy2(img, li_img_root/img.name)
            except Exception:
                pass
            try:
