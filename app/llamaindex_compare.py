...
from app.indexing import build_sparse_retriever
...
@trace_func
def build_alt_indexes(paths: List[Path], embedding_fn):
    ...
    try:
        if os.getenv("RAG_ENABLE_LLAMAINDEX", "0").lower() in ("1", "true", "yes"):
            ...
            if llx_docs:
                ...
                dense_llx = _build_chroma_at(llx_docs, embedding_fn, dir_llx, coll_llx)
                ...
                try:
                    log.info("[LLX] Built %d docs -> Chroma at %s", len(llx_docs), str(dir_llx))
                except Exception:
                    pass
    except Exception:
        pass
    ...
    try:
        if os.getenv("RAG_USE_LLAMAPARSE", "0").lower() in ("1", "true", "yes"):
            ...
            if lp_docs:
                ...
                dense_lp = _build_chroma_at(lp_docs, embedding_fn, dir_lp, coll_lp)
                ...
                try:
                    log.info("[LP] Built %d docs -> Chroma at %s", len(lp_docs), str(dir_lp))
                except Exception:
                    pass
    except Exception:
        pass
