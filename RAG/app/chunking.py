...
def structure_chunks(elements, file_path: str) -> List[Dict]:
	...
	# Advanced configuration with centralized settings and environment overrides
	try:
		# Get settings from centralized config with environment variable overrides
		split_multi = _get_chunking_setting("TEXT_SPLIT_MULTI", True)  # Default ON
		SEMANTIC = _get_chunking_setting("SEMANTIC_CHUNKING", True)  # Default ON
		
		# Text chunks: target and max tokens from config with environment overrides
		...
		# Overlap and distillation settings
		OVERLAP_N = max(0, _get_chunking_int_setting("TEXT_OVERLAP_SENTENCES", 2))  # Context window size
		DISTILL_RATIO = _get_chunking_float_setting("DISTILL_RATIO", 0.05)  # 5% distillation target
