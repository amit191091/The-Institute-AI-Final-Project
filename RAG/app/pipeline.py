...
from RAG.app.indexing import (
    build_dense_index,
    build_sparse_retriever,
    to_documents,
    dump_chroma_snapshot,
)
from RAG.app.normalized_loader import load_normalized_docs  # Optional normalized source
...
try:
    from RAG.app.graph import build_graph, render_graph_html  # type: ignore
    ...
    def render_graph_html(G, out_path: str):
        return None
try:
    from RAG.app.graphdb import build_graph_db  # type: ignore
except Exception:  # pragma: no cover
    def build_graph_db(docs):
        return 0
try:
    from RAG.app.graphdb_import_normalized import import_normalized_graph  # type: ignore
except Exception:  # pragma: no cover
...
def _discover_input_paths() -> List[Path]:
    ...
    if settings.DATA_DIR.exists():
        ...
        # Include table files from elements directory
        elements_dir = settings.DATA_DIR / "elements"
        if elements_dir.exists():
            for ext in ("*.md", "*.csv"):
                candidates.extend(elements_dir.glob(ext))
    return candidates
...
def answer_with_contexts(docs, hybrid, llm: _LLM, question: str):
    ...
    if qa.get("is_table_question"):
        ...
        for d in candidates:
            ...
            elif section == "Text" and any(keyword in content for keyword in ["w1", "w15", "w25", "w35", "wear depth", "case", "μm", "micron"]):
                filtered.append(d)
