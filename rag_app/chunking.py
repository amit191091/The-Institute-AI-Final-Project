...
def structure_chunks(elements, file_path: str) -> List[Dict]:
	...
	for el in elements:
		...
		# Use higher ratio for technical documents to preserve important details
		ratio = 0.3 if any(keyword in raw_text.lower() for keyword in ['sensor', 'accelerometer', 'tachometer', 'gear', 'failure']) else 0.15
		content = simple_summarize(raw_text, ratio=ratio)
		chunks.append(
			{
				"file_name": file_path,
				"page": page,
